...
`./run.sh` is the normal entry point. It runs `./install.sh` automatically when setup is missing, then opens the PySide6 control panel with live output and actions for the bundled recorded demo, real test suite, repair, and stop. Use `./cli.sh` for CLI-only operation and `./test.sh` to run the same tests outside the GUI.
